# app.py — Retrieval + Ranking + Clarification + Groq LLM writer
...
# ---------- API key (use secrets/env, fallback to the sample you gave) ----------
GROQ_API_KEY = (
    os.getenv("GROQ_API_KEY")
    or (st.secrets["GROQ_API_KEY"] if hasattr(st, "secrets") and "GROQ_API_KEY" in st.secrets else None)
    or "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"  # your sample; replace with secrets in prod
)
...
def llm_write_answer(user_question: str, intent_name: str, steps: str, primary_link: str = "") -> str:
    """Use Groq LLM to produce the final user-facing answer from selected steps."""
    sys = st.session_state["system_prompt_text"]
    context = (
        f"Use the following steps verbatim as constraints (do not invent new links):\n"
...
def llm_write_clarify(user_question: str, options: list[dict]) -> str:
    """Ask for disambiguation with numbered options (LLM phrasing)."""
    opts = "\n".join([f"{i+1}. {o['intent_name']} (#{o['intent_id']})" for i, o in enumerate(options)])
    sys = st.session_state["system_prompt_text"]
    prompt = (
        "The user query is ambiguous. Ask the user to pick one of these intents by number.\n"
        "Be concise and friendly. Only show the numbered list.\n\n"
...
def llm_write_oos(user_question: str) -> str:
    """Out-of-scope or safety message (LLM phrasing)."""
    sys = st.session_state["system_prompt_text"]
    prompt = (
...
def handle_user_query(user_input: str):
    ...
    # numeric choice from previous clarify
    if st.session_state.get("pending_clarify"):
        if ui.isdigit():
            idx = int(ui) - 1
            options = st.session_state["pending_clarify"]
            if 0 <= idx < len(options):
                chosen = options[idx]
                # call pipeline again to get the chosen intent's steps (cheap) or we could look up docs.json
                # Here we re-run pipeline and pick the chosen id from candidates assumption
                # Simpler: ask LLM to answer using pipeline result if available
                st.session_state.messages.append({"role": "user", "content": user_input})
                # Use pipeline to get final answer then LLM to write
                res = pipeline_answer(chosen["intent_name"])
                if res.get("type") == "answer":
                    steps = res.get("steps", "")
                    link = res.get("primary_link", "")
                    reply = llm_write_answer(ui, res.get("intent_name",""), steps, link)
                else:
                    reply = llm_write_oos(ui)
                st.session_state.messages.append({"role": "assistant", "content": reply})
                st.session_state["pending_clarify"] = []
                st.rerun()
                return
        # fall through to normal path if invalid number
    ...
    try:
        ...
        if res.get("type") == "clarify":
            st.session_state["pending_clarify"] = res.get("options", [])
            reply = llm_write_clarify(user_input, st.session_state["pending_clarify"])
        elif res.get("type") == "answer":
            steps = res.get("steps", "")
            link = res.get("primary_link", "")
            reply = llm_write_answer(user_input, res.get("intent_name",""), steps, link)
            # append top alternatives as a plain line
            alts = res.get("alternatives", [])
            if alts:
                try:
                    alt_str = " / ".join([f"{a['intent_name']}({a['rank_score']:.2f})" for a in alts])
                except Exception:
                    alt_str = " / ".join([str(a.get('intent_name','')) for a in alts])
